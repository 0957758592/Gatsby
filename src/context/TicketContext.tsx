...
function TicketProvider(props: Props) {
  ...
  const filterData = (filterData: string) => {
    setFilter(filterData);
  }

  const assignUser = async (ticketId: number, userId: number) => {
    try {
      // await backend.assign(ticketId, userId);
      const findTicket = tickets.find(ticket => ticket.id === ticketId)
      if (findTicket) {
        findTicket.assigneeId = userId
        setTickets(
          tickets.map(ticket => {
            if (ticket.id === findTicket.id) {
              return findTicket
